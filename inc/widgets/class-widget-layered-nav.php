...
if ( ! class_exists( 'WOODMART_Widget_Layered_Nav' ) ) {
	class WOODMART_Widget_Layered_Nav extends WPH_Widget {
		...
		function __construct() {
            ...
            $category_taxonomy    = get_taxonomies( array( 'name' => 'product_cat' ), 'objects' );

            if ( $category_taxonomy ) {
                $category_taxonomy = array_pop( $category_taxonomy );
                $attribute_array[ 'product_cat' ] = $category_taxonomy->label;
            }
            ...
            $attribute_array['cats']= 'cats';

            $attribute_array['product_cat']= 'product_cat';
            ...
            $categories_array_filter = $categories_array;
			...
			$args['fields'] = array(
				...
				array(
					...
					'type'    => 'dropdown',
					'std'     => 'on',
					'name'   => esc_html__( 'Collapse', 'woodmart' ),
					'fields' => array(
						esc_html__( 'OFF', 'woodmart' ) => 'off',
						esc_html__( 'ON', 'woodmart' ) => 'on',
					)
		...
		function widget( $args, $instance )	{
            ...
            if ( $instance['attribute']=='cats'){
                $taxonomy  = 'product_cat';
                $_chosen_attributes = ! empty( $_GET['filter_product_cat'] ) ? explode( ',', wc_clean( wp_unslash( $_GET['filter_product_cat'] ) ) ) : array();
                $_chosen_attributes = [
                    "product_cat"=>[
                        "terms"=>$_chosen_attributes,
                        "query_type"=>"or"
                    ]
                ];

            } else {
                $taxonomy           = isset( $instance['attribute'] ) ? ( ($instance['attribute']=='product_cat') ? 'product_cat' : wc_attribute_taxonomy_name( $instance['attribute'] ) ) : '';
			...
			if ( isset( $instance['collapse'] ) && $instance['collapse']=='on' ) $collapse = true;
			else $collapse = false;
			$instance['collapseClass'] = $collapse? 'collapse':'';

			if ( ! is_shop() && ! is_product_taxonomy() && $template == 'default' ) {
				return;
			}

			if ( isset($_GET['filter_sex']) && $instance['attribute']=='sex') {
				return;
			}
            ...
            if ($taxonomy=='product_cat'){
                $get_terms_args['exclude'] = [462, 530, 378, 182];
                $terms = get_terms( $taxonomy, $get_terms_args );
                $terms4 = get_terms( $taxonomy, ['include'=>[462, 530, 378, 182] ] );
                $terms = array_merge( $terms4, $terms );
            } else {
                $terms = get_terms( $taxonomy, $get_terms_args );
            }
        ...
        protected static function get_layered_nav_chosen_attributes(){
           $atr =  WC_Query::get_layered_nav_chosen_attributes();
           if ( ! empty( $_GET['filter_product_cat'] ) ) {
               $atr['product_cat'] = [
                   "terms"=>explode( ',', wc_clean( wp_unslash( $_GET['filter_product_cat'] ) ) ),
                   "query_type"=>"or"
               ] ;
           }
           return $atr;
		...
		protected function get_page_base_url() {
			...
			// All current filters.
			if ( $_chosen_attributes = self::get_layered_nav_chosen_attributes() ) { // phpcs:ignore Squiz.PHP.DisallowMultipleAssignments.Found, WordPress.CodeAnalysis.AssignmentInCondition.Found
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			$collapse		  	= isset( $instance['collapse'] ) ? $instance['collapse'] : 'off';
			...
			$found              = false;
//            var_dump(  $taxonomy  );
//            var_dump($_chosen_attributes[ $taxonomy ]);
			foreach ( $terms as $term ) {
				...
				if (isset($_GET['filter_sex']) AND !empty($_GET['filter_sex'])) {

					if ($term->taxonomy == 'pa_brand') {

						$count = count($this->sex_filter($term->name, $_GET['filter_sex']));
					...
					if ($term->taxonomy == 'pa_size') {
						$count = count($this->sex_filter_by_size($term->name, $_GET['filter_sex']));
				...
				// if (isset($_GET['filter_sex']) AND !empty($_GET['filter_sex'])) {
				// 	if(!preg_match('/^[а-яё]++$/ui',$term->name)){
				// 		if (preg_match('~[0-9]+~', $term->name)) {
				// 			$count = count($this->sex_filter(null, $_GET['filter_sex'], $term->name, $_chosen_attributes['product_cat']['terms']));
				// 			if (!$count) {
				// 				$filter_sex = 'nosex';
				// 			}
				// 		}
				// 		else {
				// 			$count = count($this->sex_filter($term->name, $_GET['filter_sex'], null, $_chosen_attributes['product_cat']['terms']));
				// 			if (!$count) {
				// 				$filter_sex = 'nosex';
				// 			}
				// 		}
				...
				echo '<li class="' . $filter_sex . ' wc-layered-nav-term ' . esc_attr( $class ) . '">';
		...
		private function sex_filter($brand, $sex = 'man', $size = null, $cats=[]) {
			...
			$products = new WP_Query($product_arr);
		...
		private function sex_filter_by_size($size, $sex = 'man') {
