...
function ob_add_categories_filter($filtered_posts)
{
	...
	if (!empty($_GET[$name]) && taxonomy_exists($taxonomy)) {
		...
		if (
			empty($_GET[$query_type_name]) || !in_array(strtolower($_GET[$query_type_name]), array(
				'and',
				'or'
			))
		) {
...
function truemisha_recently_viewed_products()
{
	...
	$title = '<h3 class="title slider-title">Ранее вы смотрели</h3>';
